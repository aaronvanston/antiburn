...
| M     | MCP definition exposure              | API-equivalent USD or literal input tokens | Requires attributable definition tokens and compatible-request count. Names or exposure alone are nonnumeric. Reports API-equivalent USD only when every contributing turn's model resolves in the live pricing table and the stamped pricing revision is still current; otherwise the finding still reports with literal input tokens and no dollar figure. The per-session Hygiene badge (`unusedMcpServer`) reports the same per-resource dollar figure, under the same pricing-table eligibility, next to each unused server's name; an unresolvable model still names the server with no dollar figure. The badge shows only on a finding; per the inventory policy above, it never reads clean or not assessed. |
| B     | Built-in definition replication      | API-equivalent USD or literal input tokens | Numeric for established catalog-backed replication counts. It is not converted into a confirmed win while B verification is unavailable. Same pricing-table and revision requirement as M applies for the API-equivalent USD figure; an unresolvable model still reports the token count. The per-session Hygiene badge (`unusedBuiltInTool`) reports the same per-resource dollar figure, under the same pricing-table eligibility, next to each unused tool's name; an unresolvable model still names the tool with no dollar figure. The badge shows only on a finding; per the inventory policy above, it never reads clean or not assessed. |
| K     | Injected skill document              | API-equivalent USD or literal input tokens | Requires full document tokens and compatible-request count. Listings never qualify. Same pricing-table and revision requirement as M applies for the API-equivalent USD figure; an unresolvable model still reports the token count. The per-session Hygiene badge (`unusedSkill`) reports the same per-resource dollar figure, under the same pricing-table eligibility, next to each unused skill's name; an unresolvable model still names the skill with no dollar figure. The badge shows only on a finding; per the inventory policy above, it never reads clean or not assessed. |
