...
| Claude Code                      | Built-in tool               | Auto Fix for B only           | One exact observed tool must have a matching canonical permission name and one existing standard settings deny list. The editor appends only that bare name. Wildcards, scoped rules, and general permission changes are unavailable.                                                                                                    |
