...
| `properties.detail`             | A second value from a closed vocabulary, such as a visible state, action result, `user` or `automatic` origin, `native` versus `wsl`, or a usage window's `short` or `long` role.                                                                                                                 | `ready`                                 |
| `properties.origin`             | Whether a state followed a `user` or `automatic` exposure, or whether a Burn Check watch started as `passive` or `action`. Optional and present only on `antiburn.surface_state_observed` and `antiburn.burn_check_outcome_observed`.                                                             | `user`                                  |
...
| `properties.plan`               | A learned limit factor's plan, mapped to `free`, `pro`, `max`, `max_5x`, `max_20x`, `team`, `enterprise`, `plus`, `prolite`, `business`, `edu`, `unknown` (none reported), or `other` (anything else). `max_5x` and `max_20x` apply only to Claude's reviewed Max tiers; `prolite` applies only to OpenAI. Never the provider's raw plan string. Optional and present only on `antiburn.limit_factor_observed`. | `max_5x`                                |
...
| `antiburn.setting_toggled`               | A preference changes.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | `label` — one of `live_usage`, `notifications`, `launch_at_login`, `tray_icon`, `dock_icon`, `discovery_paused`. The key only; never the value.                                                                                                                                                                                                                                                              |
...
| `antiburn.analytics_opted_out`            | You turn analytics off.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | No event-specific properties. It is emitted only when analytics was configured and enabled; an environment opt-out, unavailable configuration, or missing identity emits nothing.                                                                                                                                                                                                                                                                                     |
| `antiburn.surface_viewed`                | A native product surface becomes visible after a successful show or navigation transition. Prewarming, refreshes, remounts, resizes, and repeated show requests emit nothing.                                                                                                                                                                                                                                                                                                                                                                                                                                                | `label` — `activity`, `session_detail`, `provider_preview`, `checks_preview`, `burn_checks`, `hud`, `hud_detail`, or `settings`. `detail` — `user` or `automatic`.                                                                                                                                                                                                                                           |
...
| `antiburn.limit_factor_observed`         | A background factor-learning pass produces a learned dollars-per-percent factor for a `(provider, lane)` pair. Reports the first observation for each pair in this run, then only when its `(plan, factor band, residual band)` tuple changes, and never more than once every 24 hours for the same pair even across a genuine change. Reveals a coarse sense of plan mix and estimate accuracy per provider and lane, at worst a handful of times per install per day.                                                                                                                                                      | `label` — `anthropic`, `openai`, or `google`. `detail` — `short` or `long`, the lane. `plan` — the mapped plan vocabulary, including Claude's `max_5x` and `max_20x` and OpenAI's `prolite` where the provider reports those reviewed values. `factorBand` — the log-spaced dollars-per-percent band. `residualBand` — how far the meter and the estimate disagree. No dollar amount, percentage, account, or timestamp. |
...
| `antiburn.burn_check_auto_fix_reviewed`  | A deliberate Fix request reaches a typed preparation result. A successful result means the bounded review is visible.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | `detail` — `ready`, `stale`, `expired`, `conflict`, `unavailable`, or `failed`. No setting, current or proposed value, model, path, action ID, or private error.                                                                                                                                                                                                                                             |
| `antiburn.burn_check_auto_fix_confirmed` | The reader selects Apply change in the visible Auto Fix review.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | No event-specific properties.                                                                                                                                                                                                                                                                                                                                                                                |
| `antiburn.burn_check_auto_fix_completed` | The confirmed Auto Fix command reaches a typed result.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | `detail` — `applied_awaiting_verification`, `recovery_needed`, `stale`, `expired`, `conflict`, `unavailable`, or `failed`. No prepared-operation ID, watch ID, setting, model, path, or private error.                                                                                                                                                                                                       |
...
| `antiburn.quota_incidents_observed`      | Settings → Insights returns an assessed quota-pressure section, once per limit kind whose bucketed hit count differs from the last one reported for that kind during this run.                                                                                                                                                                                                                                                                                                                                                                                                                                                | `label` — `rolling_window`, `weekly`, `model_specific`, `weighted_usage`, `rate_limit`, or `usage_limit`. `bucket` — deduplicated hits for that limit kind. No model name, session identifier, or message text.                                                                                                                                                                          |
| `antiburn.provider_incidents_observed`   | Settings → Insights returns an assessed provider-incidents section, once per incident kind whose bucketed hit count differs from the last one reported for that kind during this run.                                                                                                                                                                                                                                                                                                                                                                                                                                         | `label` — `capacity`. `bucket` — deduplicated hits for that incident kind. No model name, session identifier, or message text.                                                                                                                                                                          |
...
The main Burn Checks workspace uses the same exposure rules. A report is
`ready` only when at least one check has a finding or a clean result; an all
unassessed report is `empty`. Hidden or inactive main-window sections do not
report exposure, state, or watch outcomes. Each `verified` or `recurred` plus
`passive` or `action` tuple is reported at most once per visible exposure, so a
large or repeatedly refreshed report cannot create per-finding volume. The
workspace can emit at most one view, four distinct surface states, four watch
outcome tuples, and the events caused by deliberate action attempts in one
exposure. Refreshes and background verification are not engagement.

Auto Fix preparation, confirmation, and command result remain separate because
each answers a different funnel step. The UI suppresses a second in-flight
request. A later retry is a new deliberate attempt. Prompt preparation occurs
at most once for one visible target action while clipboard retries can continue;
...
factor and its residual to bands. A
...
The mapping change applies to the first release that contains it. Earlier
events that report `max` or `other` cannot be split into Max tiers or Pro Lite
after delivery. Reports must segment the new values by app version.
